...
public class ToonService {
    ...
    public List<LikeToonRequest.ListRes> getList() {
        return toonRepository.findAll().stream()
                .map(LikeToonRequest.ListRes::new)
                .collect(Collectors.toList());
    }

    public Map<String, Map<String, List<LikeToonRequest.ConvertWebToonLists>>> convertList() {
        ...
        Map<String, Map<String, List<LikeToonRequest.ConvertWebToonLists>>> providerMap = new HashMap<>();

        providerMap.put("NAVER", new HashMap<>());
        providerMap.put("DAUM", new HashMap<>());

        for (Toons t : lists) {
            Map<String, List<LikeToonRequest.ConvertWebToonLists>> dayMap = providerMap.get(t.getToonProvider());
            ...
            LikeToonRequest.ConvertWebToonLists toon =
                    LikeToonRequest.ConvertWebToonLists.builder()
            ...
            for (String a : tempList) {
                List<LikeToonRequest.ConvertWebToonLists> innerList = new ArrayList<>();
